...
from gents.evaluation.model_free.errors import crps, mse
import gents.model
...
def parse_args():
    ...
    parser.add_argument(
        "--infer_batch_size", type=int, default=512, help="Batch size for inference."
    )
    parser.add_argument(
        ...
        for dataset_name in dataset_names:
            ...
            for model_name in model_names:
                ...
                if args["condition"] in model_cls.ALLOW_CONDITION:
                    ...
                    # print(ckpt['state_dict']['model.encoder.latent.backbone.c_layers.2.layer.kernel.kernel.z'])
                    # for k in ckpt:
                        # print(k)
                    # for k in ckpt['state_dict']:
                        # print(k)
                    # print('try initialize')
                    # model = model_cls(**ckpt['hyper_parameters'])
                    # for n,p in model.named_parameters():
                        # print(n, p.shape)
                    # model.load_state_dict(ckpt['state_dict'])
                    # print('finish')
                    # print(torch.load(model_ckpt))
                ...
                pd.DataFrame(avg_metrics, index=[0]).to_csv(
                    os.path.join(
                        DEFAULT_ROOT_DIR,
                        f"{model_name}_{dataset_name}_cls_sample_exp_metrics.csv",
                    ),
                    index=False,
                )
