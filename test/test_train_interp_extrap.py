...
def get_parse_args():
    parser = argparse.ArgumentParser(
        description="PyTorch CIFAR10 Training", add_help=False
    )
    # Optimizer
    ...
    parser.add_argument(
        "--config",
        default="./configs/extrapolation/vae_physionet_extrap.yaml",
        type=str,
...
def train(model, device, trainloader, optimizer, extrap=False, step=0):
    ...
    for batch_idx, data in pbar:
        step += 1
        if extrap:
            data["observed_data"] = torch.cat(
                [data["observed_data"], data["data_to_predict"]], dim=1
            )
            data["observed_tp"] = torch.cat(
                [data["observed_tp"], data["tp_to_predict"]], dim=0
            )
            data["observed_mask"] = torch.cat(
                [torch.zeros_like(data["observed_mask"]), data["mask_predicted_data"]],
                dim=1,
            )
        ...
        wandb.log(
            {f"train_{k}": v / (batch_idx + 1) for k, v in dict(log_info).items()}
        )
...
@torch.no_grad()
def eval(
    model, device, epoch, dataloader, best_mse, log_dir, classify=None, extrap=False
):
    ...
    for batch_idx, data in pbar:
        ...
        if isinstance(model, torch.nn.DataParallel):
            recon = model.module.reconstruct(
                data["observed_data"],
                data["observed_tp"],
                data["tp_to_predict"],
                masks=data["mask_predicted_data"],
                get_full_nll=False,
            )
        ...
        mse = (
            (
                (recon - data["data_to_predict"]).pow(2) * data["mask_predicted_data"]
            ).sum(1)
            / mask_sum
        ).mean()
        ...
        crps = (
            (
                crps_gaussian(
                    data["data_to_predict"].detach().cpu().numpy(),
                    mu=recon.detach().cpu().numpy(),
                    sig=sigma,
                )
                * data["mask_predicted_data"].detach().cpu().numpy()
            ).sum(1)
            / mask_sum.detach().cpu().numpy()
        ).mean()

        ## likelihood
        if extrap:
            x = torch.cat([data["observed_data"], data["data_to_predict"]], dim=1)
            tp = torch.cat([data["observed_tp"], data["tp_to_predict"]], dim=0)
            m = torch.cat(
                [torch.zeros_like(data["observed_mask"]), data["mask_predicted_data"]],
                dim=1,
            )
        ...
        if classify:
            if extrap:
                ...
                data["observed_mask"] = torch.cat(
                    [data["observed_mask"], data["mask_predicted_data"]], dim=1
                )
            if isinstance(model, torch.nn.DataParallel):
                pred = model.module.predict(
                    data["observed_data"], data["observed_tp"], data["observed_mask"]
                )
            else:
                pred = model.predict(
                    data["observed_data"], data["observed_tp"], data["observed_mask"]
    ...
    p = "TEST MSE: %.8f FULL NLL: %.8f CRPS: %.8f  " % (
        ...
        nll_loss / (batch_idx + 1),
        crps_loss / (batch_idx + 1),
    )
    print(p)
    wandb.log(
        {
            "eval_mse": mse_loss / (batch_idx + 1),
    ...
    if classify:
        ...
        if classify == "per_seq":
            ...
            auc = sklearn.metrics.roc_auc_score(
                label_all[idx_not_nan].cpu().numpy().reshape(-1),
            ...
            acc = sklearn.metrics.accuracy_score(
                label_all[mask_all].cpu().numpy(), pred_all[mask_all].cpu().numpy()
            )
...
def main(args):
    ...
    data_objs = parse_datasets(
        config.data, batch_size=args.batch_size, device=torch.device("cpu")
    )
    # Dataloaders
    ...
    if use_ema:
        ema_avg = (
            lambda averaged_model_parameter,
            model_parameter,
            num_averaged: config.optim.lamb * averaged_model_parameter
            + (1 - config.optim.lamb) * model_parameter
        )
    ...
    wandb.init(
        ...
        name=name,
        config={**vars(args), **pd.json_normalize(config, sep="_")},
        tags=[args.dataset],
    ...
    if not args.test:
        pbar = tqdm(
            range(start_epoch, start_epoch + config.optim.epochs),
