...
from ._utils import get_device, init_network_weights
...
class VAE_Baseline(nn.Module):
    def __init__(
        ...
        self.register_buffer("obsrv_std", torch.Tensor([obsrv_std]))
    ...
    def compute_all_losses(self, batch_dict, n_traj_samples=1, kl_coef=1.0):
        ...
        z0_prior = torch.distributions.Normal(
            torch.tensor(0.0).to(fp_mu.device), torch.tensor(1.0).to(fp_std.device)
        )
