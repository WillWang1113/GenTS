...
# import gc
import torch
from ._base_models import VAE_Baseline
from ._encoder_decoder import Encoder_z0_ODE_RNN, Encoder_z0_RNN
from ._utils import get_device, sample_standard_gaussian


class LatentODEorig(VAE_Baseline):
    def __init__(
        self,
        input_dim,
        latent_dim,
        ...
        super(LatentODEorig, self).__init__(
            input_dim=input_dim,
    ...
    def sample_traj_from_prior(self, time_steps_to_predict, n_traj_samples=1):
        ...
        z0_prior = torch.distributions.Normal(
            torch.tensor(0.0).to(time_steps_to_predict.device),
            torch.tensor(1.0).to(time_steps_to_predict.device),
        )

        # Sample z0 from prior
        latent_dim = self.latent_dim * 2 if self.use_poisson_proc else self.latent_dim
        starting_point_enc = z0_prior.sample(
            [n_traj_samples, 1, latent_dim]
        ).squeeze(-1)
