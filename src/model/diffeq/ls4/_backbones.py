...
class Decoder(nn.Module):
    def __init__(self, config, sigma, z_dim, in_channels, bidirectional):
        ...
        self.latent = Model(**config.prior)
        self.bidirectional = bidirectional
        if self.bidirectional:
            self.latent_back = Model(**config.prior)
            config.decoder.d_input = config.decoder.d_input * 2

        self.dec = Model(**config.decoder)
    ...
    def sample(self, bs, t_vec, device="cuda"):
        ...
        for t in range(seq_len):
            ...
            # prior
            z_t, hidden_state_z = self.latent.step(
                z_t,
                None,
                t=None if isinstance(t_vec, int) else t_vec[t],
                state=hidden_state_z,
                sample=True,
            )
            ...
            if self.bidirectional:
                z_t_back = zs_back[-1]
                z_t_back, hidden_state_z_back = self.latent_back.step(
                    z_t_back,
                    None,
                    t=None if isinstance(t_vec, int) else t_vec[seq_len - t - 1],
                    state=hidden_state_z_back,
                )
        ...
        if self.config.decoder['aux_channels'] == 0:
            ...
            for t in range(seq_len):
                ...
                x_t, hidden_state_x = self.dec.step(
                    zs[:, t],
                    x_t,
                    t=None if isinstance(t_vec, int) else t_vec[t],
                    state=hidden_state_x,
                )
    ...
    def extrapolate(self, x_given, t_vec, t_vec_pred, z_post):
        ...
        # xs = [self.x_prior[None].expand(bs,-1)]
        z_prior = torch.cat(
            [self.z_prior[None, None].expand(bs, -1, -1), z_post[:, :-1]], dim=1
        )
        hidden_state_z = self.latent.default_state(bs, device=device)
        ...
        for t in range(z_prior.shape[1]):
            # prior
            _, _, _, hidden_state_z = self.latent.step(
                z_prior[:, t], None, t=t_vec[t], state=hidden_state_z, sample=False
            )
        ...
        for t in range(len(t_vec_pred)):
            ...
            # prior
            z_t, z_t_mean, z_t_std, hidden_state_z = self.latent.step(
                z_t, None, t=t_vec_pred[t], state=hidden_state_z, sample=False
            )
        ...
        if self.config.decoder['aux_channels'] == 0:
            ...
            xs = torch.cat(
                [self.x_prior[None, None].expand(bs, -1, -1), x_given[:, :-1]], dim=1
            )
            hidden_state_x = self.dec.default_state(bs, device=device)

            for t in range(len(t_vec)):
                _, hidden_state_x = self.dec.step(
                    zs[:, t], xs[:, t], t=t_vec[t], state=hidden_state_x, sample=True
                )
    ...
    def reconstruct(self, x, t_vec, z_post, z_post_back=None):  # decoder.reconstruct
        ...
        x_input = torch.cat(
            [self.x_prior[None, None].expand(x.shape[0], -1, -1), x[:, :-1]], dim=1
        )
        ...
        if self.bidirectional and z_post_back is not None:
            ...
            x, x_std = self.decode(
                torch.cat([z_post, z_post_back], dim=-1), x_input, t_vec
            )
    ...
    def forward(self, x, t_vec, z_post, z_post_back=None):
        ...
        z_input = torch.cat(
            [self.z_prior[None, None].expand(z_post.shape[0], -1, -1), z_post[:, :-1]],
            dim=1,
        )
        ...
        if self.bidirectional and z_post_back is not None:
            ...
            x_input_back = torch.cat(
                [
                    self.x_prior_back[None, None].expand(x.shape[0], -1, -1),
                    x.flip(1)[:, :-1],
                ],
                dim=1,
            )
            prior_mean_back, prior_std_back = self.latent_back(
                z_input_back, x_input_back, t=t_vec.flip(-1)
            )

            z_post_back_flip = z_post_back.flip(1)
            dec_mean, dec_std = self.decode(
                torch.cat([z_post, z_post_back_flip], dim=-1), x_input, t_vec
            )
...
class Encoder(nn.Module):
    def __init__(self, config, z_dim, bidirectional):
        ...
        self.use_spatial = config.use_spatial
        self.bidirectional = bidirectional
        self.latent = Model(**config.posterior)

        if self.bidirectional:
            self.latent_back = Model(**config.posterior)

    def _reparameterized_sample(self, mu, std):
        eps = torch.empty(
            size=std.size(), device=mu.device, dtype=torch.float
        ).normal_()
...
class VAE(nn.Module):
    ...
    def forward(self, x, timepoints, masks, labels=None, plot=False, sum=False):
        # B,L = x.shape[:2]
        log_info = {}
        z_post, z_post_mean, z_post_std = self.encoder.encode(
            x, timepoints, use_forward=True
        )  # (B, L , z_dim)

        if self.bidirectional:
            z_post_back, z_post_mean_back, z_post_std_back = self.encoder.encode(
                x, timepoints, use_forward=False
            )  # (B, L , z_dim)
            ...
            (
                ...
                z_prior_mean_back,
                z_prior_std_back,
            ) = self.decoder(x, timepoints, z_post, z_post_back)  # (B, L, z_dim)

            kld_loss_forward = self._kld_gauss(
                z_post_mean, z_post_std, z_prior_mean, z_prior_std, masks, sum=sum
            )  # (B,)
            kld_loss_back = self._kld_gauss(
                z_post_mean_back,
                z_post_std_back,
                z_prior_mean_back,
                z_prior_std_back,
            ...
            log_info.update(
                {
                    "z_post_mean_forward": z_post_mean.mean().detach().cpu().item(),
            ...
            dec_mean, dec_std, z_prior_mean, z_prior_std = self.decoder(
                x, timepoints, z_post
            )  # (B, L, z_dim)
        ...
        if plot:
            full_nll = self._kld_gauss(
                z_post_mean, z_post_std, z_prior_mean, z_prior_std, masks, sum=True
        ...
        log_info.update(
            {
                ...
                "mse_loss": self._mse(dec_mean, x, masks, sum=False)
                .mean()
                .detach()
                .cpu()
                .item(),
    ...
    def reconstruct(
        self, x, t_vec, t_vec_pred=None, x_full=None, masks=None, get_full_nll=False
    ):
        ...
        if t_vec_pred is None:
            ...
            dec_mean, dec_std, z_prior_mean, z_prior_std = self.decoder.extrapolate(
                x, t_vec, t_vec_pred, z
            )  # (B, L, z_dim)
            ...
            if get_full_nll:
                ...
                kld_loss = self._kld_gauss(
                    z_mean_pred, z_std_full, z_prior_mean, z_prior_std, masks, sum=True
                )  # (B,)

                full_nll = kld_loss + self._nll_gauss(
                    dec_mean, dec_std, x_full[:, -len(t_vec_pred) :], masks, sum=True
                )
                full_nll = full_nll.mean()
    ...
    def interpolate(
        ...
        z2, z2_mean, z2_std = self.encoder.encode(
            x2, t_vec, use_forward=True
        )  # (bs, T, z_dim)
        z_post_back = None
        assert not self.bidirectional, f"not supposed to be bidirectional"

        # do mixing / interpolation in latent
        alpha_ts = self._get_alpha_ts(
            self.config.interp_schedule,
    ...
    def _kld_gauss(self, mean_1, std_1, mean_2, std_2, masks=None, sum=False):
        ...
        if masks is None:
            ...
            if sum:
                ...
                loss = (
                    0.5
                    * torch.sum((kld_element * masks[..., None]).mean(-1), dim=-1)
                    / masks_sum
                )
        return loss
    ...
    def full_nll(self, x, timepoints, masks=None):
        ...
        full_nll = kld_loss + self._nll_gauss(dec_mean, dec_std, x, masks, sum=True)
        full_nll = full_nll.mean()
...
def binary_ce_loss(pred, labels):
    ...
    if torch.sum(labels == 0.0) == 0 or torch.sum(labels == 1.0) == 0:
        print(
