...
class S4DKernel(nn.Module):
    ...
    def step(self, u, state):
        ...
        self.dC = C  # (C H N)
        self.dB = self.dC.new_ones(self.H, self.N) * (torch.exp(dtA) - 1.0) / A  # (H N)
...
class S4DJointKernel(nn.Module):
    ...
    def step(self, u, x, state, **kwargs):
        ...
        self.dB = (
            self.dC_main.new_ones(self.H, self.N) * (torch.exp(dtA) - 1.0) / A
        )  # (H N)
        self.dE = E * (torch.exp(dtA) - 1.0) / A
        next_state = (
            contract("h n, b h n -> b h n", self.dA, state)
...
class S4DJoint(nn.Module):
    ...
    def forward(
        ...
        y_main = (
            yu_main
            + yx_main
            + u * self.D_main.unsqueeze(-1)
            + x * self.F_main.unsqueeze(-1)
        )
        y_aux = (
            yu_aux
            + yx_aux
            + u * self.D_aux.unsqueeze(-1)
            + x * self.F_aux.unsqueeze(-1)
        )
    ...
    def step(self, ins, state, **kwargs):
        ...
        y = (
            y
            + contract("bh,h->bh", u, self.D_main)
            + contract("bh,h->bh", x, self.F_main)
        )
        y_aux = (
            y_aux
            + contract("bh,h->bh", u, self.D_aux)
            + contract("bh,h->bh", x, self.F_aux)
        )
        y = self.activation(y)
        y = self.output_linear(y.unsqueeze(-1)).squeeze(-1)
