...
from ._s4d import S4D, S4DJoint
from ._seq_unet import SequentialUnet, s4_registry
...
class Model(nn.Module):
    def __init__(
        ...
        if self.d_temb > 0:
            self.temb = nn.Sequential(
                torch.nn.Linear(self.d_temb * 2, self.d_temb * 4),
                torch.nn.SiLU(),
                torch.nn.Linear(self.d_temb * 4, self.d_temb),
            )
        ...
        if use_latent:
            if latent_type == "none":
                ...
                self.linear = nn.Linear(d_model, d_output)
                self.mean = s4_registry[s4_type](
                    d_model=d_output,
                    d_state=d_state,
                    transposed=True,
                    lr=min(0.001, lr),
                )
                self.log_std = s4_registry[s4_type](
                    d_model=d_output,
                    d_state=d_state,
                    transposed=True,
                    lr=min(0.001, lr),
            ...
            if self.n_outheads > 1:
                self.linear = nn.ModuleList(
                    [nn.Linear(d_model, d_output) for _ in range(self.n_outheads)]
                )
            else:
                self.linear = nn.Linear(d_model, d_output)
    ...
    def forward(self, x, aux=None, t=None):
        ...
        if (
            aux is not None
            and self.use_aux
            and (self.s4_type == "s4d_joint" or self.mix_aux)
        ):
    ...
    def default_state(self, *batch_shape, device="cuda"):
        ...
        if self.use_latent and self.latent_type == "split":
            ...
            return {
                "unet": unet_state,
                "latent": (latent_default_mean, latent_default_std),
            }
    ...
    def step(self, x, aux=None, state: dict = None, t=None, sample=False):
        ...
        if (
            ...
            aux = self.aux_encoder_in(aux)
            (x, aux), unet_next_state = self.backbone.step(
                (x, aux), state=state["unet"], t=t
            )
        elif aux is not None and self.use_aux:
            aux = self.aux_encoder_in(aux)
            x = torch.cat([x, aux], dim=-1)
        ...
        if self.use_latent:
            if self.latent_type == "single":
                ...
                x, next_state_ = self.latent.step(
                    x, state=state["latent"], t=t, sample=True
                )
                ...
                x_log_std, next_state_std = self.log_std.step(
                    x, state=state["latent"][1]
                )
                ...
                eps = torch.empty(
                    size=x_std.size(), device=x_std.device, dtype=torch.float
                ).normal_()
