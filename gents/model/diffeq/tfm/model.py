...
from ._backbones import (
    mse_loss,
    MLP_conditional_memory,
    MLP_conditional_memory_sde_noise,
)
from ._utils import SDE_func_solver, torch_wrapper_tv, metrics_calculation


class Noise_MLP_Cond_Memory_Module(BaseModel):
    def __init__(
        self,
        treatment_cond,
        ...
        super().__init__()
        self.flow_model = MLP_conditional_memory(
            dim=seq_dim,
            w=w,
        ...
        if implementation == "SDE":
            self.noise_model = MLP_conditional_memory_sde_noise(
                dim=seq_dim,  # @TODO: give \hat{x_1}?
                w=w,
                time_varying=time_varying,
                conditional=conditional,
                treatment_cond=treatment_cond,
            ...
            self.noise_model = MLP_conditional_memory(
                dim=seq_dim,  # @TODO: give \hat{x_1}?
                w=w,
                time_varying=time_varying,
                conditional=conditional,
        ...
        self.sigma = sigma
        self.naming = (
            "Noise_MLP_Cond_memory_Module_" + implementation
            if naming is None
            else naming
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        x0, x0_class, x1, x0_time, x1_time = (
            self.__convert_tensor__(x0),
            self.__convert_tensor__(x0_class),
            self.__convert_tensor__(x1),
            self.__convert_tensor__(x0_time),
            self.__convert_tensor__(x1_time),
        )
        ...
        if self.implementation == "SDE":
            ...
            loss = self.loss_fn(
                xt[:, : self.dim] + noise.clone().detach(), x1
            ) + self.loss_fn(xt[:, -1], futuretime)
            uncertainty = xt[:, : self.dim].clone().detach() + noise
            noise_loss = self.loss_fn(uncertainty, x1)
        else:
            loss = self.loss_fn(xt[:, : self.dim], x1) + self.loss_fn(
                xt[:, -1], futuretime
            )
            uncertainty = torch.abs(xt[:, : self.dim].clone().detach() - x1)
            # noise model incorporation (model loss)
            noise_loss = self.loss_fn(
                self.noise_model.forward_train(in_tensor)[:, : self.dim], uncertainty
            )
    ...
    def configure_optimizers(self):
        ...
        self.noise_optimizer = torch.optim.Adam(
            self.noise_model.parameters(), lr=self.lr
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        Returns:
            _type_: _description_
        """
        loss, pairs, metricD, noise_loss, noise_pair = self.test_func_step(
            batch, batch_idx, mode="val"
        )
        self.log("val_loss", loss, on_epoch=True, on_step=False, sync_dist=True)
        self.log(
            "noise_val_loss", noise_loss, on_epoch=True, on_step=False, sync_dist=True
        )
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log(
            "noise_test_loss", noise_loss, on_epoch=True, on_step=False, sync_dist=True
        )
        for key, value in metricD.items():
    ...
    def test_func_step(self, batch, batch_idx, mode="none"):
        ...
        full_traj = torch.cat(
            [x0_values[0, 0, : self.dim].unsqueeze(0), x1_values[0, :, : self.dim]],
            dim=0,
        )
    ...
    def test_trajectory_ode(self, pt_tensor):
        ...
        """
        node = NeuralODE(
            torch_wrapper_tv(self.flow_model),
            solver="dopri5",
            sensitivity="adjoint",
            atol=1e-4,
            rtol=1e-4,
        ...
        node_noise = NeuralODE(
            torch_wrapper_tv(self.noise_model),
            solver="dopri5",
            sensitivity="adjoint",
            atol=1e-4,
            rtol=1e-4,
        ...
        for i in range(len_path):
            time_span = self.__convert_tensor__(
                torch.linspace(times_x0[i], times_x1[i], 10)
            ).to(x0_values.device)

            new_x_classes = torch.cat(
                [
                    x0_classes[i][: -(self.memory * self.dim)].unsqueeze(0),
                    time_history.unsqueeze(0),
                ],
                dim=1,
            )
            with torch.no_grad():
                ...
                if i == 0:
                    testpt = torch.cat(
                        [x0_values[i].unsqueeze(0), new_x_classes], dim=1
                    )
                else:  # incorporate last prediction
                    testpt = torch.cat([pred_traj, new_x_classes], dim=1)  # noqa: F821
                # print(testpt.shape)
            ...
            mse_traj = (
                self.loss_fn(pred_traj, ground_truth_coords).detach().cpu().numpy()
            )
            mse.append(mse_traj)
            uncertainty_traj = ground_truth_coords - pred_traj
            noise_mse_traj = (
                self.loss_fn(noise_traj, uncertainty_traj).detach().cpu().numpy()
            )
            noise_mse.append(noise_mse_traj)

            # history update
            flattened_coords = pred_traj.flatten()
            time_history = torch.cat(
                [time_history[self.dim :].unsqueeze(0), flattened_coords.unsqueeze(0)],
                dim=1,
            ).squeeze()
    ...
    def _sde_solver(self, sde, initial_state, time_span):
        ...
        for t in time_span[1:]:
            ...
            current_state = (
                current_state + drift * dt + diffusion * noise
            )  # @NEED this or not?
