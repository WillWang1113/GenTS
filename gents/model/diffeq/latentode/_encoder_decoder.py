...
class Encoder_z0_RNN(nn.Module):
    ...
    def forward(self, data, time_steps, run_backwards=True):
        ...
        if self.use_delta_t:
            ...
            delta_t = torch.cat((delta_t, torch.zeros(1).to(delta_t)))
...
class Encoder_z0_ODE_RNN(nn.Module):
    ...
    def __init__(
        ...
        if GRU_update is None:
            self.GRU_update = GRU_unit(latent_dim, input_dim, n_units=n_gru_units)
        else:
    ...
    def forward(self, data, time_steps, run_backwards=True, save_info=False):
        ...
        if len(time_steps) == 1:
            prev_y = torch.zeros((1, n_traj, self.latent_dim)).to(data)
            prev_std = torch.zeros((1, n_traj, self.latent_dim)).to(data)
