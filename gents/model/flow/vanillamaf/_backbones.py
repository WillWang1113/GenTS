...
class MaskedLinear(nn.Module):
    def __init__(
        ...
        if condition in ['predict', 'impute']:
            ...
            self.cond_linear = nn.Embedding(cond_in_features, out_features)
        else: ...
...
class MADE(nn.Module):
    ...
    def __init__(
        ...
        self.joiner = MaskedLinear(
            num_inputs, num_hidden, input_mask, num_cond_inputs, condition
        )

        self.trunk = nn.Sequential(
            act_func(),
            MaskedLinear(num_hidden, num_hidden, hidden_mask, condition),
            act_func(),
            MaskedLinear(num_hidden, num_inputs * 2, output_mask, condition),
        )
