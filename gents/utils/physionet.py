...
import gents.utils.latentode_utils as utils
import numpy as np
import tarfile
import torch
...
#     dataset = PhysioNet(
#         "data/physionet", train=False, download=True, device=torch.device("cuda:0")
#     )
#     args = Namespace(
#         dataset="physionet",
#         extrap=False,
...
#         cut_tp=None,
#     )
#     data_min, data_max = get_data_min_max(dataset)
...
#         print(batch['mask_predicted_data'])
        
#         fig, axs = plt.subplots(4, 1, figsize=(10, 5))
#         axs = axs.flatten()
#         axs[0].plot(batch['observed_mask'].cpu().numpy()[0,:,17])
#         axs[1].plot(batch['observed_data'].cpu().numpy()[0,:,17])
#         axs[2].plot(batch['mask_predicted_data'].cpu().numpy()[0,:,17])
#         axs[3].plot(batch['data_to_predict'].cpu().numpy()[0,:,17])
#         fig.savefig('physionet.png')
#         break        
