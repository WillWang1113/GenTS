...
def initialize_ts2vec(input_dims, X_train, device, ts2vec_path=None):
    config = dict(batch_size=8, lr=0.001, output_dims=100, max_train_length=3000)
    model = TS2Vec(input_dims=input_dims, device=device, **config)
    if X_train is None and ts2vec_path is None:
        raise ValueError("Either X_train or ts2vec_path must be provided.")
    elif X_train is None and ts2vec_path is not None:
        model.load(ts2vec_path)
        print("load ts2vec model from path:", ts2vec_path)
        return model
    elif X_train is not None and ts2vec_path is None:
        print("train a new ts2vec model")
        model.fit(X_train, verbose=False)
        return model
    else:  # both X_train and ts2vec_path are provided
        try:
            model.load(ts2vec_path)
            print("load ts2vec model from path:", ts2vec_path)
            print("re-train a new ts2vec model with provided X_train")
        except:
            print("fail to load ts2vec model from path:", ts2vec_path)
            print("train a new ts2vec model")
            model.fit(X_train, verbose=False)
            model.save(ts2vec_path)
        return model

    # if ts2vec_path is not None:
    #     try:
    #         model.load(ts2vec_path)
    #         print("load ts2vec model from path:", ts2vec_path)
    #     except:
...
class TS2Vec:
    ...
    def fit(self, train_data, n_epochs=None, n_iters=None, verbose=False):
        ...
        if n_iters is None and n_epochs is None:
            n_iters = (
                200 if train_data.shape[0] <= 100000 else 600
